'''
Ajout des ennemis
On empêche le scroll des mobiles et on ajoute les collisions
On règle aussi le pb des mobiles qui ne sont pas au 1er plan
'''
import pygame
import random
from os import path
...
# part16/11: pour gérer les layers
PLAYER_LAYER = 2
PLATFORM_LAYER = 1
POWERUP_LAYER = 1
MOB_LAYER = 2
...
MOB_FREQ = 5000     # (en ms) part16/5: fréquence d'apparition des mobiles
...
class Player(pygame.sprite.Sprite):
    def __init__(self, platforms):
        self._layer = PLAYER_LAYER  # part16/12
...
class Platform(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, width, height):
        self._layer = PLATFORM_LAYER    # part16/13
...
class Powerup(pygame.sprite.Sprite):
    def __init__(self, platform):
        self._layer = POWERUP_LAYER     # part16/14
...
# part16/1: définition des Mobiles
class Mob(pygame.sprite.Sprite):
    def __init__(self):
        self._layer = MOB_LAYER     # part16/15
    ...
    # part16/3: déplacement du Mobile
    def update(self):
        self.rect.x += self.vx
        self.vy += self.dy
        ...
        center = self.rect.center      # part16/4 à décommenter
        if self.dy < 0:
            self.image = self.image_up
        else:
            self.image = self.image_down
        self.rect = self.image.get_rect()  # part16/4 à décommenter
        self.rect.center = center          # part16/4 à décommenter
        self.rect.y += self.vy
...
while want_to_play:    
    ...
    show_start_screen()     
    # part16/16: on utilise des Sprites avec layers pour gérer les niveaux
    # de visibilité plus simplement
    #all_sprites = pygame.sprite.Group()
    all_sprites = pygame.sprite.LayeredUpdates()
    all_platforms = pygame.sprite.Group()
    all_powerups = pygame.sprite.Group()
    all_mobs = pygame.sprite.Group()    # part16/2
    score = 0
    mob_timer = 0                       # part16/7
    ...
    while running:
        ...
        # part16/6: génère un nouveau mobile ?
        now = pygame.time.get_ticks()
        if now - mob_timer > MOB_FREQ + random.choice([-1000, -500, 0, 500, 1000]):
            mob_timer = now
            Mob()
            # part16/8: tester et trouver le pb: quand l'écran scrolle les Mob suivent le joueur !
        
        # part16/19: collision avec les mobiles ?
        ...
        if player.rect.top <= HEIGHT / 4:
            ...
            # part16/10: il faut aussi déplacer vers le bas les mobiles
            for mob in all_mobs:
        ...
        # part16/17: ligne inutile avec les layers
