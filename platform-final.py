'''
On nettoie aussi le code des __init__ sans width et height
Correction du bug des pf qui se chevauchent (on tombe sur la + basse) 
Les plate-formes bougent et les powerups et le joueur bougent avec
'''
import pygame
import random
from os import path
...
PLATFORM_LIST = [
    ...
    #(WIDTH / 2 - 50 + 30, HEIGHT * 3 / 4 - 30),
    (125, HEIGHT - 350),
    (350, 200),
    (175, 100),    
]
PLATFORM_TIMELIFE = 6000
...
class Player(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        # suit la pf si elle bouge
        if self.platform:
            self.pos.x += self.platform.vx
    ...
    def animate(self):
        ...
        if not self.jumping:
            # mesure le temps d'attente sur la plateforme
            if self.platform:
                if pygame.time.get_ticks() - self.wait_time > self.platform.timelife / 2:
                    if not self.platform.half_broken:
                        half_broken_sound.play()
                        self.platform.show_broken()
                        self.platform.half_broken = True
                    
                if pygame.time.get_ticks() - self.wait_time > self.platform.timelife:   
                    broken_sound.play()
                    self.platform.kill()
                    self.platform = None
...
class Cloud(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.groups = all_sprites, all_clouds
        super().__init__(self.groups)
        ...
        # taille aléatoire pour les nuages
    ...
    def update(self):
        # change le test pour afficher les nuages quand le joueur tombe
...
class Platform(pygame.sprite.Sprite):
    def __init__(self, x, y, must_move=True):
        ...
        self.image_nu = random.randint(0, len(images) - 1)
        self.image = images[self.image_nu]
        self.image.set_colorkey(BLACK)
        self.rect = self.image.get_rect()
        self.rect.x = x
        self.rect.y = y
        self.vx = 0
        self.timelife = PLATFORM_TIMELIFE
        self.broken = False
    ...
    def show_broken(self):
        if not self.broken:
            # charges les images des pf fissurées
            broken_images = [
                spritesheet.get_image(0, 384, 380, 94),
                spritesheet.get_image(382, 204, 200, 100),
            ] 
            self.broken = True
            self.image = broken_images[self.image_nu]
            self.image.set_colorkey(BLACK)
...
def new_platform():
    # on veut être sûr que les plateformes ne se chevauchent pas
    # car c'est difficile à gérer - pas implémenté
    while True:
        ...
        collision = False
        hits = pygame.sprite.spritecollide(p, all_platforms, False)
        for hit in hits:
            if hit == p:
                continue
            collision = True
...
def load_images():
    global spritesheet, cloud_images
    dir = path.dirname(__file__)
    ...
    for i in range(1, 4):
        cloud_images.append(pygame.image.load(path.join(img_dir, f'cloud{i}.png')).convert())
    
# charge les sons
jump_sound = None
boost_sound = None
half_broken_sound = None
broken_sound = None

def load_sounds():
    global jump_sound, boost_sound, half_broken_sound, broken_sound
    dir = path.dirname(__file__)
    ...
    half_broken_sound = pygame.mixer.Sound(path.join(snd_dir, "expl1.wav")) 
    half_broken_sound.set_volume(.2)
    broken_sound = pygame.mixer.Sound(path.join(snd_dir, "expl2.wav")) 
    broken_sound.set_volume(.2)
...
while want_to_play:    
    ...
    while running:
        ...
        # test de collision avec le pf mais uniquement si le joueur tombe
        if player.vel.y > 0:
            ...
            if hits:
                ...
                # si le joueur est sur une nouvelle plateforme, on prend
                # le temps courant
                if player.platform != lowest:
                    player.wait_time = pygame.time.get_ticks()
                ...
                if lowest.rect.left - 10 < player.pos.x < lowest.rect.right + 10:
                    if player.pos.y < lowest.rect.centery:
                        player.pos.y = lowest.rect.top  - 40   # pour ne pas donner l'impression que le joueur s'enfonce dans la plateforme quand il saute
                        
                        # mais ça ne suffit pas: le joueur s'enfonce lentement dans la plateforme
                        # que faut-il faire en plus ?
                        player.vel.y = 0
                        player.jumping = False
                        player.platform = lowest
        ...
        if player.rect.top <= HEIGHT / 4:
            # génère un nouveau nuage aléatoirement
            ...
            # on déplace les nuages vers le bas mais moins rapidement
            for cloud in all_clouds:
                # ajout vitesse aléatoire
                cloud_speed = random.randrange(1, 5)
                #cloud.rect.y += max(abs(player.vel.y / 2), 2)
                cloud.rect.y += max(abs(player.vel.y / cloud_speed), 2)
        ...
        # Crée de nouvelles PF pour en avoir toujours 5, sauf si le jeu est fini
        if not end_of_game:
